...
inline std::string resolvePath(const std::string& filePath) {
    ...
    if (!exeDir.empty()) {
        ...
        for (int i = 0; i < 6; ++i) {
            ...
            // Also check under NovaForge/Client/App/ (source-tree location
            // for client shaders when running without a post-build copy)
            resolved = parent / "NovaForge" / "Client" / "App" / filePath;
            if (std::filesystem::exists(resolved)) {
                return resolved.string();
            }
        }
